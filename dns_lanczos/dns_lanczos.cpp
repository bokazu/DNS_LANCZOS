...
double dns_lancozs_N(char info_ls, const int mat_dim, int tri_mat_dim,
                     double *M, std::string outfile_name)
{
    int count = 0;
    ...
    double *
        tri_diag_evec;  // lapack_dstevに渡す用('N'の場合は参照されないので適当に用意しておく)
    ...
    for (int ls = 0; ls < tri_mat_dim; ls++)
    {
        if (err_checker)
        {
            ...
            if (ls % 2 == 0)
            {
                cblas_dgemv(CblasColMajor, CblasNoTrans, mat_dim, mat_dim, 1.0,
                            M, mat_dim, u[0], 1, 1.0, u[1], 1);
            ...
            if (ls > 0)
            {
                ...
                if (info_ls == 'y')
                {
                    std::cout << "eps = " << std::setprecision(16) << eps
                              << std::endl;
                }
    ...
    if (count % 2 == 0)
        gstate_eval = eval_even[0];
    else
        gstate_eval = eval_odd[0];
...
double dns_lancozs_V(char info_ls, const int mat_dim, int tri_mat_dim,
                     double *M, std::string outfile_name, char c,
                     std::string testfile_name)
{
    ...
    double *tri_diag_evec = new double[tri_mat_dim * tri_mat_dim];
    vec_init(tri_mat_dim * tri_mat_dim, tri_diag_evec);
    ...
    for (int ls = 0; ls < count + 2; ls++)
    {
        ...
        if (ls % 2 == 0)
        {
            cblas_dgemv(CblasColMajor, CblasNoTrans, mat_dim, mat_dim, 1.0, M,
